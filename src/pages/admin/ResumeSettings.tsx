    ...
    const fetchSettings = async () => {
      try {
        ...
        if (!settingsRes.ok || !scoringRes.ok || !templatesRes.ok) {
          setError("Failed to fetch resume settings");
          setLoading(false);
          return;
        }
        ...
        settingsData.forEach((setting: { setting_key: string; setting_value: string | number }) => {
          switch (setting.setting_key) {
            case 'max_file_size':
              transformedSettings.general.maxFileSize = Number(setting.setting_value);
              break;
            case 'allowed_formats':
              transformedSettings.general.allowedFormats = Array.isArray(setting.setting_value) 
                ? setting.setting_value as string[] 
                : String(setting.setting_value).split(',').map(s => s.trim());
              break;
            case 'auto_analysis':
              transformedSettings.general.autoAnalysis = Boolean(setting.setting_value);
              break;
            case 'email_notifications':
              transformedSettings.general.emailNotifications = Boolean(setting.setting_value);
              break;
            case 'analysis_timeout':
              transformedSettings.general.analysisTimeout = Number(setting.setting_value);
              break;
          }
        });
