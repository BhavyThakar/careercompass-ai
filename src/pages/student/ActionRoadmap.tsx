...
// TypeScript interfaces for ActionRoadmap
interface Task {
  title: string;
  completed: boolean;
  type: string;
...
const ActionRoadmap = () => {
  const [actionRoadmap, setActionRoadmap] = useState<RoadmapPhase[]>([]);
      ...
      try {
        ...
        if (token) {
          ...
          if (response.ok) {
            ...
            const transformedData = Object.values(data as Record<string, { phases: { id: string; title: string; description: string; status: string; milestones: { title: string; completed: boolean }[] }[] }>).flatMap((roadmap) => {
              return roadmap.phases.map((phase: { id: string; title: string; description: string; status: string; milestones: { title: string; completed: boolean }[] }) => ({
                id: phase.id,
                month: phase.title,
                phase: phase.title,
                progress: phase.status === 'completed' ? 100 : phase.status === 'active' ? 50 : 0,
                tasks: phase.milestones.map((milestone: { title: string; completed: boolean }) => ({
            ...
            <Card className={month.progress > 0 ? "border-primary/20" : ""}>
              ...
              <CardContent>
                <div className="grid sm:grid-cols-2 gap-3">
                  {month.tasks.map((task: Task, taskIndex: number) => {
